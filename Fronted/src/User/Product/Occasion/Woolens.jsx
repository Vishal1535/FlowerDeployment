...
import { WoolenBouquetItem } from "../../../Component/Hero/SpecialBouquet/Woolen/WoolenBouquetItem";

export const Woolens = ({ woolens = [] }) => {
  const [search, setSearch] = useState("");

  if (!woolens.length) return null;
  ...
  const filteredWoolens = woolens.filter((woolen) => {
    ...
    return (
      matches(woolen?.name) ||
      matches(woolen?.description) ||
      matches(woolen?.category) ||
      matches(woolen?.occasion) ||
      matches(woolen?.color) ||
      matches(woolen?.size) ||
      matches(woolen?.type) ||
      matches(woolen?.material) ||
      matches(woolen?.price)
    );
  });
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <input
                ...
                placeholder="Search woolen bouquets..."
                className="
                  flex-1
                  min-w-0
                  h-full
          ...
          className={`
            ...
            ${
              filteredWoolens.length === 1
                ...
                : filteredWoolens.length === 2
                  ? "grid-cols-1 sm:grid-cols-2 max-w-[660px] mx-auto"
                  : filteredWoolens.length === 3
